...
class NanoRpcHandler:
    def __init__(self, url, nanoto_api_key):
        self.rpc = NanoRpc(url=url, node_version=NodeVersion.V25_0)
        self.nanoto_api_key = nanoto_api_key
    ...
    async def receivable(self, account, count="5", source="true"):
        return await self.rpc.receivable(account, count=count, source=source)

    async def frontiers(self, account, count=1):
        return await self.rpc.frontiers(account, count=count)

    async def work_generate(self, hash, use_peers="true"):
        return await self.rpc.work_generate(hash, use_peers=use_peers)

    async def account_info(self, account):
        return await self.rpc.account_info(account)

    async def blocks_info(self, blocks, source=True):
        return await self.rpc.blocks_info(blocks, source=source)

    async def process(self, block, json_block=False):
        return await self.rpc.process(block, json_block=json_block)
    ...
    async def confirmation_info(self, json_block, root, contents=None, representatives=None):
        return await self.rpc.confirmation_info(json_block=json_block, root=root, contents=contents, representatives=representatives)
    ...
    async def database_txn_tracker(self, min_read_time, min_write_time):
        return await self.rpc.database_txn_tracker(min_read_time=min_read_time, min_write_time=min_write_time)
    ...
    async def frontiers(self, account, count):
        return await self.rpc.frontiers(account, count)
    ...
    async def pending(self, account, count, threshold=None, source=None, include_active=None, min_version=None, sorting=None, include_only_confirmed=None):
        return await self.rpc.pending(account, count, threshold=threshold, source=source, include_active=include_active, min_version=min_version, sorting=sorting, include_only_confirmed=include_only_confirmed)
    ...
    async def stats(self, type):
        return await self.rpc.stats(type=type)
    ...
    async def unchecked(self, json_block, count):
        return await self.rpc.unchecked(json_block=json_block, count=count)

    async def unchecked_clear(self):
        return await self.rpc.unchecked_clear()

    async def unchecked_get(self, json_block, hash):
        return await self.rpc.unchecked_get(json_block=json_block, hash=hash)

    async def unchecked_keys(self, json_block, key, count):
        return await self.rpc.unchecked_keys(json_block=json_block, key=key, count=count)
    ...
    async def receivable(self, account, count, threshold=None, source=None, include_active=None, min_version=None, sorting=None, include_only_confirmed=None, offset=None):
        return await self.rpc.receivable(account, count, threshold=threshold, source=source, include_active=include_active, min_version=min_version, sorting=sorting, include_only_confirmed=include_only_confirmed, offset=offset)
